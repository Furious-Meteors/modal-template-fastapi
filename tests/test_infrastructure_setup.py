...
def _reset_otel_globals():
    ...
    import src.infrastructure.setup as setup_mod
...
def test_setup_telemetry_no_op_when_endpoint_missing(monkeypatch):
    ...
    from src.infrastructure.setup import setup_telemetry
...
def test_setup_telemetry_installs_tracer_provider_on_no_op(monkeypatch):
    ...
    monkeypatch.delenv("OTEL_EXPORTER_OTLP_ENDPOINT", raising=False)
    from src.infrastructure.setup import setup_telemetry, get_tracer
    setup_telemetry()
    tracer = get_tracer()
...
def test_setup_telemetry_installs_meter_provider_on_no_op(monkeypatch):
    ...
    monkeypatch.delenv("OTEL_EXPORTER_OTLP_ENDPOINT", raising=False)
    from src.infrastructure.setup import setup_telemetry, get_meter
    setup_telemetry()
    meter = get_meter()
...
def test_setup_telemetry_idempotent_with_endpoint(monkeypatch):
    ...
    with patch("src.infrastructure.setup.OTLPSpanExporter", return_value=MagicMock()), \
         patch("src.infrastructure.setup.OTLPMetricExporter", return_value=MagicMock()):
        setup_mod.setup_telemetry()
...
def test_grafana_headers_empty_when_no_credentials(monkeypatch):
    ...
    from src.infrastructure.setup import _grafana_headers
...
def test_build_resource_service_name_from_env(monkeypatch):
    ...
    from src.infrastructure.setup import _build_resource
...
def test_setup_telemetry_span_exporter_gets_traces_path(monkeypatch):
    ...
    with patch("src.infrastructure.setup.OTLPSpanExporter", mock_span_exporter_cls), \
         patch("src.infrastructure.setup.OTLPMetricExporter", return_value=MagicMock()):
        setup_mod.setup_telemetry()
...
def test_setup_telemetry_metric_exporter_gets_metrics_path(monkeypatch):
    ...
    with patch("src.infrastructure.setup.OTLPSpanExporter", return_value=MagicMock()), \
         patch("src.infrastructure.setup.OTLPMetricExporter", mock_metric_exporter_cls):
        setup_mod.setup_telemetry()
...
def test_setup_telemetry_passes_auth_headers_to_exporters(monkeypatch):
    ...
    with patch("src.infrastructure.setup.OTLPSpanExporter", mock_span_cls), \
         patch("src.infrastructure.setup.OTLPMetricExporter", mock_metric_cls):
        setup_mod.setup_telemetry()
...
def test_setup_telemetry_metric_export_interval_from_env(monkeypatch):
    ...
    with patch("src.infrastructure.setup.OTLPSpanExporter", return_value=MagicMock()), \
         patch("src.infrastructure.setup.OTLPMetricExporter", return_value=MagicMock()), \
         patch("src.infrastructure.setup.PeriodicExportingMetricReader", mock_reader_cls):
        setup_mod.setup_telemetry()
